...
class FeatureGeneretor:
    def __init__(self, scenario: Scenario, annotations: list[AnnotationEntry], window_size: int) -> None:
        if window_size % 2 != 0:
            raise ValueError('Window size must be even')

        self.window_size = window_size
        self.scenario = scenario
        self.lane_lines_shp = self._road_multiline(scenario=scenario)
        self.annotations = annotations

        self.model = None


    def _road_multiline(self, scenario):
        road_shp = list(scenario.road.lanes.values())
    ...
    def _compute_input_matrix_for_vehicle(self, vehicle: Vehicle) -> np.ndarray:
        closest_distance_to_line_line = self._closest_distance_to_lane_line(s=vehicle.s,
                                                                           d=vehicle.d)
    ...
    def _compute_input_matrix_for_vehicle_with_potential(self, vehicle: Vehicle) -> np.ndarray:
        ...
        X = np.empty((0, 3 * (self.window_size + 1)))
        ...
        for f in range(len(vehicle.frames)):
            ...
            x = np.hstack((x_dist, x_vel, x_pot))
            X = np.vstack((X, x))
    ...
    def get_potential_feature(self, ego_id: int, frame: int) -> float:
        ...
        for i, v in enumerate(vehs):
            ...
            U = np.exp(eta*relative_velocity)/ (2* np.pi * kn(0, eta*relative_velocity)) * np.exp(-relative_distance**2/2)/(2*np.pi)
    ...
    def _closest_distance_to_lane_line(self, s, d):
        aaa = []
        for ss, dd in zip(s, d):
            aaa.append(
                self.lane_lines_shp.distance(Point(ss, dd))
            )
